...
try {
  ...
  exit('Unable to prepare HR staff profile table.');
}

// Load employee
$stmt = $pdo->prepare("SELECT e.*, d.name AS department_name, t.name AS team_name
...
// Load profile
$stmt = $pdo->prepare("SELECT * FROM hr_staff_profiles WHERE employee_id=? LIMIT 1");
$stmt->execute([$employeeId]);
$profileRow = $stmt->fetch(PDO::FETCH_ASSOC);

$profile = [];
if ($profileRow && !empty($profileRow['profile_json'])) {
  $decoded = json_decode((string)$profileRow['profile_json'], true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$profileRow) {
    $ins = $pdo->prepare("INSERT INTO hr_staff_profiles
      (employee_id, application_id, profile_json, created_by_admin_id, updated_by_admin_id, created_at, updated_at)
      VALUES (?, NULL, ?, ?, ?, NOW(), NOW())");
    $ins->execute([$employeeId, $json ?: '{}', (int)$user['id'], (int)$user['id']]);
  } else {
    $upd = $pdo->prepare("UPDATE hr_staff_profiles SET profile_json=?, updated_by_admin_id=?, updated_at=NOW() WHERE employee_id=? LIMIT 1");
    $upd->execute([$json ?: '{}', (int)$user['id'], $employeeId]);
  }

  header('Location: ' . admin_url('staff-hr.php?id=' . $employeeId . '&tab=' . urlencode($tab) . '&saved=1'));
  exit;
}
...
$empName = trim((string)($employee['nickname'] ?: trim((string)($employee['first_name'] . ' ' . $employee['last_name']))));
if ($empName === '') $empName = 'Employee #' . (int)$employeeId;
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <p class="mt-1 text-sm text-slate-600">
                ...
                <?php if (!empty($employee['employee_code'])): ?>
                  <span class="text-slate-400">·</span> <span class="font-semibold text-slate-900"><?= h2((string)$employee['employee_code']) ?></span>
                <?php endif; ?>
              </p>
            ...
            <div class="flex items-center gap-2">
              <a href="<?= h(admin_url('employee-edit.php?id=' . (int)$employeeId)) ?>"
                 class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold hover:bg-slate-50">← Staff details</a>
            </div>
          </div>

          <div class="mt-4 flex flex-wrap gap-2">
            <?php foreach ($tabs as $k => $label): ?>
              <a href="<?= h(admin_url('staff-hr.php?id=' . (int)$employeeId . '&tab=' . $k)) ?>"
