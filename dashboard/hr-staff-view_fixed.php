          ...
          <div class="flex items-start gap-4">
            <div class="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
              <?php if (!empty($s['photo_path'])): ?>
                <img src="<?= h2(admin_url('hr-staff-photo.php?staff_id=' . (int)$staffId)) ?>" alt="Staff photo" class="h-full w-full object-cover">
              <?php else: ?>
        ...
        <aside class="space-y-4">
            ...
            <div class="mt-3">
              <?php if (!empty($s['photo_path'])): ?>
                <div class="mb-3 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                  <img src="<?= h2(admin_url('hr-staff-photo.php?staff_id=' . (int)$staffId)) ?>" alt="Staff photo" class="w-full object-cover">
                </div>
              <?php endif; ?>
          ...
          <section class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="flex items-center justify-between">
              ...
              <a class="text-xs font-semibold text-slate-700 hover:text-slate-900"
                 href="<?= h2(admin_url('hr-staff-document.php?staff_id=' . (int)$staffId)) ?>">Manage</a>
            </div>
            ...
            <form method="post" enctype="multipart/form-data" class="mt-3 space-y-2">
              ...
              <select name="doc_type" class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm" required>
                ...
                <?php foreach (['rtw'=>'Right to work','dbs'=>'DBS','cv'=>'CV','ref'=>'Reference','training'=>'Training','other'=>'Other'] as $k => $label): ?>
                  <option value="<?= h2($k) ?>"><?= h2($label) ?></option>
                <?php endforeach; ?>
              </select>

              <input type="file" name="document_file" class="block w-full text-sm" required>
