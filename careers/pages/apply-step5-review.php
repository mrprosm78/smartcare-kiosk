...
$qsBase = ['token' => $token];
if (!empty($jobSlug)) { $qsBase['job'] = $jobSlug; }
if (!function_exists('sc_apply_step_url')) {
  function sc_apply_step_url(array $base, int $step): string {
    $q = $base;
    $q['step'] = (string)$step;
    return 'apply.php?' . http_build_query($q);
  }
}
...
if (count($cleanJobs) === 0) {
    $warnings[] = ['type'=>'missing', 'label'=>'Work history: no jobs added', 'step'=>2];
}
...
if (empty(trim($edu['highest_education_level'] ?? ''))) {
    $warnings[] = ['type'=>'missing', 'label'=>'Education: highest level is missing', 'step'=>3];
}
...
if (count($nonEmptyRefs) < 2) {
    $warnings[] = ['type'=>'missing', 'label'=>'References: please add at least 2 references', 'step'=>4];
}

// Checks
if (empty(trim($checks['has_right_to_work'] ?? ''))) {
    $warnings[] = ['type'=>'missing', 'label'=>'Checks: right to work answer is missing', 'step'=>1];
}
if (empty(trim($checks['has_current_dbs'] ?? ''))) {
    $warnings[] = ['type'=>'missing', 'label'=>'Checks: DBS status is missing', 'step'=>1];
}
?>
...
<div class="space-y-5 text-[11px]">
            ...
            <ul class="mt-3 space-y-2">
                <?php foreach ($warnings as $w): ?>
                    <li class="flex items-start justify-between gap-3">
                        ...
                        <a href="<?= h(sc_apply_step_url($qsBase, (int)$w['step'])); ?>" class="text-[11px] font-medium text-sc-primary hover:text-sc-primary">
                            Fix
                        </a>
                    </li>
                <?php endforeach; ?>
    ...
    <div class="rounded-2xl border border-sc-border bg-white px-4 py-4">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="<?= h(sc_apply_step_url($qsBase, 1)); ?>" class="text-[11px] font-medium text-sc-primary hover:text-sc-primary">Edit</a>
        </div>
        ...
        <div class="mt-3 grid gap-3 md:grid-cols-2">
            ...
            <div class="space-y-1">
                ...
                <p class="font-medium text-slate-900"><?= scv($personal['phone'] ?? ''); ?></p>
            </div>
    ...
    <div class="rounded-2xl border border-sc-border bg-white px-4 py-4">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="<?= h(sc_apply_step_url($qsBase, 2)); ?>" class="text-[11px] font-medium text-sc-primary hover:text-sc-primary">Edit</a>
        </div>
    ...
    <div class="rounded-2xl border border-sc-border bg-white px-4 py-4">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="<?= h(sc_apply_step_url($qsBase, 3)); ?>" class="text-[11px] font-medium text-sc-primary hover:text-sc-primary">Edit</a>
        </div>
    ...
    <div class="rounded-2xl border border-sc-border bg-white px-4 py-4">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="<?= h(sc_apply_step_url($qsBase, 4)); ?>" class="text-[11px] font-medium text-sc-primary hover:text-sc-primary">Edit</a>
        </div>
    ...
    <div class="pt-1 flex items-center justify-between">
        <a
            href="<?= h(sc_apply_step_url($qsBase, 4)); ?>"
        ...
        <a
            href="<?= h(sc_apply_step_url($qsBase, 6)); ?>"
