          ...
          <form method="post" class="mt-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <section class="lg:col-span-2 rounded-3xl border border-slate-200 bg-white p-5">
              ...
              <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-semibold text-slate-600">Employee code (optional)</label>
                  <input name="employee_code" value="<?= h((string)($employee['employee_code'] ?? '')) ?>" class="mt-1 w-full rounded-2xl bg-white border border-slate-200 px-3 py-2 text-sm" placeholder="e.g. EMP-001">
                </div>
              ...
              <div class="mt-4">
                ...
                <div>
                  ...
                  <input name="first_name" value="<?= h((string)($employee['first_name'] ?? '')) ?>" class="mt-1 w-full rounded-2xl bg-white border border-slate-200 px-3 py-2 text-sm" placeholder="e.g. Aisha">
                </div>

                <div>
                  <label class="block text-xs font-semibold text-slate-600">Last name</label>
                  <input name="last_name" value="<?= h((string)($employee['last_name'] ?? '')) ?>" class="mt-1 w-full rounded-2xl bg-white border border-slate-200 px-3 py-2 text-sm" placeholder="e.g. Khan">
                </div>
                ...
                <div>
                  <label class="block text-xs font-semibold text-slate-600">PIN (<?= $isNew ? 'required' : 'leave blank to keep' ?>)</label>
                  <input name="pin" inputmode="numeric" autocomplete="new-password" class="mt-1 w-full rounded-2xl bg-white border border-slate-200 px-3 py-2 text-sm" placeholder="4+ digits">
                  <div class="mt-1 text-xs text-slate-500">Stored securely as a hash.</div>
                </div>

                <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:col-span-2">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="text-xs text-slate-500">Use for agency workers. Name field will show agency label.</div>
                  </div>
                  <input name="agency_label" value="<?= h((string)($employee['agency_label'] ?? '')) ?>" class="w-40 rounded-2xl bg-white border border-slate-200 px-3 py-2 text-sm" placeholder="Agency" />
                </div>
                ...
                <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:col-span-2">
                  ...
                  <div>
                    ...
                    <div class="text-xs text-slate-500">Inactive employees cannot clock in/out.</div>
                  </div>
                </div>
              </div>
            </section>
