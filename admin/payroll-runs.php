        ...
        <main class="flex-1 min-w-0">
          <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <div class="mt-1 text-sm text-white/70">Run monthly payroll batches (hours + audit snapshots). Overtime is calculated weekly; the last incomplete week of the month is deferred to the next month.</div>
            </div>
          </div>
          ...
          <?php endif; ?>
          <?php if ($success): ?>
            <div class="mt-4 p-3 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-100"><?php echo h($success); ?></div>
          <?php endif; ?>
            ...
            <section class="rounded-3xl border border-white/10 bg-white/5 p-4">
              ...
              <form method="get" class="mt-3 flex items-end gap-3">
                <label class="block flex-1">
                  <div class="text-sm text-white/70">Month</div>
                  <input type="month" name="ym" value="<?php echo h($ym); ?>" class="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2">
                </label>
              ...
              <?php if ($canRun): ?>
                <div class="mt-5 border-t border-white/10 pt-4">
                  ...
                  <form method="post" class="mt-3 space-y-3">
                    ...
                    <label class="block">
                      <div class="text-sm text-white/70">Reason / note (optional)</div>
                      <input name="reason" class="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2" placeholder="e.g. Monthly run" />
                    </label>
              ...
              <?php else: ?>
                <div class="mt-5 text-sm text-white/60">You can view batches, but your role cannot run payroll.</div>
              <?php endif; ?>
            </section>

            <section class="lg:col-span-2 rounded-3xl border border-white/10 bg-white/5 p-4">
                  ...
                  <tbody>
                    <?php foreach ($batches as $b): ?>
                      <tr class="border-t border-white/10">
                        <td class="py-2">
                          <a class="text-emerald-200 hover:underline" href="<?php echo h(admin_url('payroll-view.php?batch_id='.(int)$b['id'])); ?>">#<?php echo (int)$b['id']; ?></a>
                        </td>
                    ...
                    <?php endforeach; ?>
                    <?php if (!$batches): ?>
                      <tr><td colspan="5" class="py-3 text-white/60">No payroll batches yet.</td></tr>
                    <?php endif; ?>
                  </tbody>
                </table>
              </div>
