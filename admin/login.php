...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($username === '' || $password === '') {
    ...
    if (!$user || !password_verify($password, (string)$user['password_hash'])) {
      ...
      $stmt = $pdo->prepare("INSERT INTO admin_sessions (session_id, user_id, device_id, ip_address, user_agent, created_at, last_seen_at) VALUES (?,?,?,?,?,UTC_TIMESTAMP,UTC_TIMESTAMP) ON DUPLICATE KEY UPDATE user_id=VALUES(user_id), device_id=VALUES(device_id), ip_address=VALUES(ip_address), user_agent=VALUES(user_agent), last_seen_at=UTC_TIMESTAMP, revoked_at=NULL, revoke_reason=NULL");
      $stmt->execute([$sessionId, (int)$user['id'], (int)$device['id'], $ip ?: null, $ua ?: null]);
  ...
  <div class="min-h-dvh flex flex-col">
    <header class="px-4 sm:px-6 pt-6 pb-4">
      <div class="max-w-xl mx-auto">
        ...
        <p class="mt-2 text-sm text-white/70">This device is authorised. Sign in to continue.</p>
      </div>
    </header>
