...
require_once __DIR__ . '/db.php'; // adjust if already included elsewhere

// Resolve base path safely
if (defined('APP_BASE_PATH') && APP_BASE_PATH !== '') {
    $basePath = APP_BASE_PATH;
} else {
    // Auto-detect from URL: parent of /kiosk
    $script = $_SERVER['SCRIPT_NAME'];          // e.g. /name_kiosk/kiosk/index.php
    $basePath = rtrim(dirname(dirname($script)), '/'); // /name_kiosk
}
...
<head>
  ...
  <link rel="stylesheet" href="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/assets/kiosk.css?v=<?=$v?>">
...
<body class="no-select bg-slate-950 text-white min-h-dvh">
  ...
  <?php
    // Runtime base path injection for kiosk JS (prevents hardcoded URLs).
    // Prefer private config APP_BASE_PATH; fallback to auto-detect from SCRIPT_NAME.
    $scriptName = (string)($_SERVER['SCRIPT_NAME'] ?? '');
    $detectedDir = rtrim(str_replace('\\', '/', dirname($scriptName)), '/');
    $detectedBase = rtrim(str_replace('\\', '/', dirname($detectedDir)), '/');
  ...
  <!-- ✅ Runtime config injected from PHP -->
  <script>
    window.SMARTCARE = {
      basePath: "<?= htmlspecialchars($basePath, ENT_QUOTES) ?>",
      apiBase: "<?= htmlspecialchars($basePath, ENT_QUOTES) ?>"
    };
  </script>

  <!-- ✅ Versioned JS assets -->
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.config.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.dom.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.toast.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.idb.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.camera.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.photo.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.crypto.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.api.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.sync.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.ui.js?v=<?=$v?>"></script>
  <script src="<?= htmlspecialchars($basePath, ENT_QUOTES) ?>/js/kiosk.main.js?v=<?=$v?>"></script>
</body>
</html>
